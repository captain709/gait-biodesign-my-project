...
left_position_foot_path = "../pkl_peam_test/position_graph/LeftFoot"
...
left_position_hand_path = "../pkl_peam_test/position_graph/LeftHand"
...
for speedy in treadmill_speed:
    ...
    for person in folder_name:
        ...
        excel_data_velocity_z = pd.read_excel(file_name, sheet_name = 'Segment Position')[['Left Foot z','Pelvis z']]
        excel_data_velocity_z['LeftFootToPelvisz'] = excel_data_velocity_z['Left Foot z'] - excel_data_velocity_z['Pelvis z']
        excel_data_velocity_z = excel_data_velocity_z.iloc[60:361]
        ...
        excel_data_velocity_z_list = excel_data_velocity_z['LeftFootToPelvisz'].values.tolist()
        print(type(excel_data_velocity_z_list))
        print(len(excel_data_velocity_z_list))
        print('z' + person+ speedy + 'done')
        list_of_excel_data_velocity_z.append(excel_data_velocity_z_list)
        print(len(list_of_excel_data_velocity_z))
        
   
    # print(list_of_excel_data_velocity_x(0))
    mean_of_excel_data_velocity_z = []
    for i in range(0,n):
        sum_of_num = 0
        for j in range(0,10):
            
            sum_of_num = sum_of_num + float(list_of_excel_data_velocity_z[j][i])
            mean_of_num = sum_of_num/len(folder_name)
        mean_of_excel_data_velocity_z.append(mean_of_num)
        print('mean_z' + speedy + 'done')
    
    all_mean_data_final.append(mean_of_excel_data_velocity_z)


    

    sd_of_excel_data_velocity_z = []
    for i in range (0,n):
        what_the_sigma = 0
        for j in range(0,10):
            what_the_sigma = what_the_sigma + ((list_of_excel_data_velocity_z[j][i]- mean_of_excel_data_velocity_z[i])**2)
            total_sd = math.sqrt(what_the_sigma / n)
        sd_of_excel_data_velocity_z.append(total_sd)
        print('sd_z' + speedy + 'done' )

    all_sd_data_final.append(sd_of_excel_data_velocity_z)

print(len(all_mean_data_final))
print(len(mean_of_excel_data_velocity_z))
print(len(all_sd_data_final))
print(len(sd_of_excel_data_velocity_z))


for k in range(0,5):
    excel_frame_velocity_x = pd.read_excel(file_name, sheet_name = 'Segment Position')[['Frame']]
    excel_frame_velocity_x = excel_frame_velocity_x.iloc[60:361]
    ...
    plt.ylabel('MeanLeftFootToPelvisz_vel (Y-axis)')
    plt.title('meanleftFootToPelvisz_vel')
    plt.savefig(op.join(left_position_foot_path, f"MeanPositionLeftFoot_Z_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_position_foot_path, f"MeanPositionLeftFoot_Z_{k +1 }.png")}")
...
for k in range(0,5):
    ...
    plt.xlabel('Frame (X-axis)')
    plt.ylabel('SDLeftFootToPelvisz_vel (Y-axis)')
    plt.title('SDleftFootToPelvisz_vel')
    plt.savefig(op.join(left_position_foot_path, f"SDPositionLeftFoot_Z_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_position_foot_path, f"SDPositionLeftFoot_Z_{k +1 }.png")}")
...
for k in range (0,5):
    ...
    plt.ylabel('MeanLeftFootToPelvisz_vel (Y-axis)')
    plt.title('meanleftFootToPelvisz_vel')
    plt.savefig(op.join(left_position_foot_path, f"MeanPositionLeftFoot_with_SD_Z_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_position_foot_path, f"MeanPositionLeftFoot_with_SD_Z_{k +1 }.png")}")
