...
for speedy in treadmill_speed:
    ...
    for person in folder_name:
        ...
        excel_data_velocity_x = pd.read_excel(file_name, sheet_name = 'Segment Velocity')[['Left Foot x','Pelvis x']]
        excel_data_velocity_x['LeftFootToPelvisx'] = excel_data_velocity_x['Left Foot x'] - excel_data_velocity_x['Pelvis x']
        excel_data_velocity_x = excel_data_velocity_x.iloc[60:131]
        ...
        excel_data_velocity_x_list = excel_data_velocity_x['LeftFootToPelvisx'].values.tolist()
...
for k in range(0,5):
    ...
    plt.ylabel('MeanLeftFootToPelvisx_vel (Y-axis)')
    plt.title('meanleftFootToPelvisx_vel')
    plt.savefig(op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_X_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_X_{k +1 }.png")}")
...
for k in range(0,5):
    ...
    plt.xlabel('Frame (X-axis)')
    plt.ylabel('SDLeftFootToPelvisx_vel (Y-axis)')
    plt.title('SDleftFootToPelvisx_vel')
    plt.savefig(op.join(left_velocity_foot_path, f"SDVelocityLeftFoot_X_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_velocity_foot_path, f"SDVelocityLeftFoot_X_{k +1 }.png")}")
...
for k in range (0,5):
    ...
    plt.ylabel('MeanLeftFootToPelvisx_vel (Y-axis)')
    plt.title('meanleftFootToPelvisx_vel')
    plt.savefig(op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_with_SD_X_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_with_SD_X_{k +1 }.png")}")
