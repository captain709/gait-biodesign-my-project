...
for speedy in treadmill_speed:
    ...
    for person in folder_name:
        ...
        excel_data_velocity_z = pd.read_excel(file_name, sheet_name = 'Segment Velocity')[['Left Foot z','Pelvis z']]
        excel_data_velocity_z['LeftFootToPelvisz'] = excel_data_velocity_z['Left Foot z'] - excel_data_velocity_z['Pelvis z']
        excel_data_velocity_z = excel_data_velocity_z.iloc[60:131]
...
for k in range(0,5):
    ...
    plt.savefig(op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_Z_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_Z_{k +1 }.png")}")
...
for k in range(0,5):
    ...
    plt.savefig(op.join(left_velocity_foot_path, f"SDVelocityLeftFoot_Z_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_velocity_foot_path, f"SDVelocityLeftFoot_Z_{k +1 }.png")}")
...
for k in range (0,5):
    ...
    plt.ylabel('MeanLeftFootToPelvisz_vel (Y-axis)')
    plt.title('meanleftFootToPelvisz_vel')
    plt.savefig(op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_with_SD_Z_{k + 1}.png"))
    
    print(f"Plot saved at: {op.join(left_velocity_foot_path, f"MeanVelocityLeftFoot_with_SD_Z_{k +1 }.png")}")
